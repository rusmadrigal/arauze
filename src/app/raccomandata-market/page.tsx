...
import { RACCOMANDATA_SEEDS } from "@/lib/seo/keywordSeeds";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title:
      data?.metaTitle ??
      "Raccomandata Market Italia 2026 – codici, mittenti e schede analizzate",
    description:
      data?.metaDescription ??
      "Consulta l’elenco aggiornato delle Raccomandate Market analizzate da Arauze, con schede dedicate e collegamenti interni ai codici pubblicati.",
...
export default async function RaccomandataMarketPage() {
  ...
  const seedItems: RaccomandataItem[] = RACCOMANDATA_SEEDS.map((item) => ({
    _id: `seed-${item.code}`,
    code: item.code,
  ...
  const byCode = new Map<string, RaccomandataItem>();
  for (const item of [...items, ...seedItems]) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4" aria-label="Elenco Raccomandata Market Italia">
      <div className="rounded-2xl shadow-card bg-white p-6 md:p-10">
        <TopNav />
        <PageBreadcrumbs
          items={[
            { label: "Home", href: "/" },
            { label: "Raccomandata Market" },
          ]}
        />
        ...
        <section className="mt-10 grid gap-4 md:grid-cols-3">
          ...
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              Collega il mercato delle raccomandate ai codici F24 più cercati.
            </p>
          </Link>
          <Link
            ...
            className="rounded-2xl border border-slate-200 bg-slate-50 p-5 transition hover:border-slate-300 hover:bg-white"
          >
            <h2 className="text-base font-semibold text-slate-900">Centri CMP</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              Approfondisci i centri di meccanizzazione postale più rilevanti.
            </p>
          </Link>
          <Link
            href="/codice-tributo/3918"
            className="rounded-2xl border border-slate-200 bg-slate-50 p-5 transition hover:border-slate-300 hover:bg-white"
          >
            <h2 className="text-base font-semibold text-slate-900">Codici fiscali chiave</h2>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              Inizia dai codici più cercati per IMU, TARI e tributi principali.
            </p>
          </Link>
        </section>
