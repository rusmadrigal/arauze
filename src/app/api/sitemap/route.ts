...
import { CODICE_TRIBUTO_SEEDS, RACCOMANDATA_SEEDS } from "@/lib/seo/keywordSeeds";
...
type SanitySitemapPayload = {
  ...
  codiceTributo?: Array<{ slug?: string; kind?: "code" | "guide"; _updatedAt?: string }>;
};
...
export async function GET() {
  ...
  const codiceTributoDocs = data.codiceTributo ?? [];
  ...
  const raccomandataSeedEntries: SitemapEntry[] = RACCOMANDATA_SEEDS.map((item) => ({
    loc: `${base}/raccomandata/${encodeURIComponent(item.code.toLowerCase())}`,
    lastmod: lastmodFromSanity(item._updatedAt),
    changefreq: "weekly",
    priority: 0.7,
  }));

  const codiceTributoSeedEntries: SitemapEntry[] = CODICE_TRIBUTO_SEEDS.map((item) => ({
    loc: `${base}/codice-tributo/${encodeURIComponent(item.slug.toLowerCase())}`,
    lastmod: lastmodFromSanity(item.updatedAt),
    changefreq: "weekly",
    priority: item.kind === "code" ? 0.8 : 0.7,
  }));

  const codiceTributoDocsEntries: SitemapEntry[] = codiceTributoDocs
    .map((item) => {
      const slug = String(item.slug ?? "").trim().toLowerCase();
      if (!slug) return null;
      return {
        loc: `${base}/codice-tributo/${encodeURIComponent(slug)}`,
        lastmod: lastmodFromSanity(item._updatedAt),
        changefreq: "weekly" as const,
        priority: item.kind === "code" ? 0.8 : 0.7,
      } satisfies SitemapEntry;
  ...
  const entries: SitemapEntry[] = [
    ...
    ...raccomandataEntries,
    ...raccomandataSeedEntries,
    ...cmpDetailEntries,
    ...codiceTributoDocsEntries,
    ...codiceTributoSeedEntries,
  ];
