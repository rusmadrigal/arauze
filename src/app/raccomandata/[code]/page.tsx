...
export async function generateMetadata({
  ...
  const sanityPage = await sanityClient.fetch<RaccomandataPageDoc>(
    RACCOMANDATA_BY_CODE,
    { code },
    { next: { revalidate } }
  );
  const page = sanityPage ?? getRaccomandataSeed(code);

  if (!page) {
    return {
      title: "Raccomandata",
      description: raccomandataMetaDescriptionFallback(code),
      alternates: alternatesItalianCanonical(
        `${getSiteOrigin()}/raccomandata/${code.toLowerCase()}`
      ),
    };
  }
...
export async function generateStaticParams(): Promise<Array<{ code: string }>> {
  const pages = await sanityClient.fetch<Array<{ code?: string }>>(
    RACCOMANDATA_LIST,
    {},
    { next: { revalidate } }
  );
  const codes = new Set<string>(
    [
      ...(pages ?? []).map((page) => (page.code ?? "").trim().toLowerCase()),
      ...RACCOMANDATA_SEEDS.map((page) => page.code.trim().toLowerCase()),
    ].filter(Boolean)
  );
...
export default async function RaccomandataPage({
  ...
  const [chartData, reportCount, relatedCandidates] = await Promise.all([
    ...
    sanityClient
      .fetch<number>(REPORTS_BY_CODE, { code: codice }, { next: { revalidate } })
      ...
      <div className="rounded-2xl shadow-card bg-white p-6 md:p-10">
        <div className="space-y-8 md:space-y-10">
          ...
          <PageBreadcrumbs
            items={[
              { label: "Home", href: "/" },
              { label: "Raccomandata Market", href: "/raccomandata-market" },
              { label: `Codice ${codice}` },
            ]}
          />
          ...
          <section className="grid gap-4 md:grid-cols-3">
            ...
            >
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                Vedi l&apos;elenco completo delle schede e i codici più cercati.
              </p>
            </Link>
            <Link
              href="/codice-tributo"
            ...
            >
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                Vai al hub fiscale se stai cercando F24, IMU o TARI.
              </p>
            </Link>
            <Link
            ...
            >
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                Collega il codice al centro di smistamento postale più rilevante.
              </p>
            </Link>
          </section>
