...
export default function CmpHomeClient({ initialList }: Props) {
  ...
  const [filter, setFilter] = useState<"Tutte" | "Nord" | "Centro" | "Sud">("Tutte");
        ...
        <div className="space-y-10">
          ...
          <section className="grid gap-7 md:grid-cols-2">
            {filteredList.map((cmp) => {
              const locParts = [
                cmp.city
                  ? `${cmp.city}${cmp.province ? ` (${cmp.province})` : ""}`
                  : null,
                cmp.region,
                cmp.macroArea,
              ].filter(Boolean);
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <h2 className="text-lg font-semibold text-gray-900">{cmp.name}</h2>
