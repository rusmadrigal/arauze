...
function StatCard({
  ...
  return (
    <li className="min-w-0">
      <div
        className="flex h-full min-h-23 gap-3 rounded-2xl border border-gray-200/90 bg-white p-3.5 shadow-sm sm:min-h-0 sm:p-4"
        role="group"
        aria-label={summary}
      >
        <div
        ...
        <div className="min-w-0 flex-1 font-sans">
          ...
          <p className="mt-1 text-[11px] font-normal leading-snug text-gray-800 sm:text-xs">
            {value}
          </p>
          {subline ? (
            <p className="mt-0.5 text-[10px] font-normal leading-snug text-gray-500 sm:text-[11px]">
              {subline}
            </p>
          ) : null}
        </div>
...
export default function RaccomandataTrustPanel({
  ...
  return (
    <aside
      className="rounded-2xl border border-gray-200/80 bg-linear-to-b from-gray-50/95 to-gray-100/40 px-4 py-4 shadow-sm md:px-6 md:py-5"
      aria-label="Informazioni sulla scheda e sulle fonti"
    >
      <div className="flex gap-3 sm:gap-4">
        <span className="rac-section-icon mt-0.5 bg-white sm:h-11 sm:w-11">
          <ShieldCheck className="h-5 w-5 text-emerald-600" aria-hidden />
        </span>
        <div className="min-w-0 flex-1">
          <p className="rac-card-heading">Informazioni orientative</p>
          <p className="rac-body mt-1.5">
            ...
            . Non sostituisce l&apos;avviso di mancata consegna, il piego, la raccomandata
            o le comunicazioni ufficiali del mittente: verifica sempre su documenti
            cartacei, su poste.it (se applicabile) e sui canali istituzionali.
          </p>
        </div>
      </div>

      <div className="my-4 h-px w-full bg-gray-200/90 sm:my-5" aria-hidden />
