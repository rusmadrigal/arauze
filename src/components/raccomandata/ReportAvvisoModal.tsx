...
export default function ReportAvvisoModal({ open, onClose, initialCode = "" }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!/^\d{3,6}$/.test(code.trim()) || !mittente.trim()) {
    ...
    try {
      const res = await fetch("/api/submit-report", {
        ...
        body: JSON.stringify({
          code: code.trim(),
          mittenteSegnalato: mittente.trim(),
      ...
      {done ? (
        <div className="text-sm">
          ...
          <p className="text-gray-600">
            La tua segnalazione è stata ricevuta e verrà revisionata. Questo aiuta a migliorare il database di Arauze.
          </p>
        </div>
      ) : (
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Mittente visto</label>
            <input
              value={mittente}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Data ricezione (opz.)</label>
              <input
                type="date"
                value={dataRicezione}
